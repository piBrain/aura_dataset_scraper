...
class DataCleanerTest(unittest.TestCase):
    ...
    def test_simple_case(self):
        simple_case = {'api_url': 'example.com/api' ,'original_text': '', 'request':[['GET','this/will/pass/inspection']] }
        self.cleaner._simple_case(simple_case).should.eql( { 'api_url': 'example.com/api', 'method': 'GET', 'arguments':
            { 'data':'' },'parsed_request':'example.com/this/will/pass/inspection' } )

    def test_multiple_simple_case(self):
        multiple_simple_case = {'original_text': '', 'api_url': 'example.com/api', 'request':[['GET','this/will/pass/inspection'],['POST','this/too']] }
        valid = [
                { 'api_url': 'example.com/api', 'method': 'GET', 'arguments': { 'data':'' }, 'parsed_request':'example.com/this/will/pass/inspection' },
                { 'api_url': 'example.com/api', 'method': 'POST', 'arguments': { 'data':'' }, 'parsed_request':'example.com/this/too' }
        ]
    ...
    def test_simple_curl_case(self):
        simple_curl = {'data':'', 'api_url': 'http://example.com/api', 'request':[''], 'original_text': 'curl http://www.example.com'}
        (self.cleaner._curl_format_case(simple_curl)
             .should.eql({
                 'api_url': 'http://example.com/api',
                 'method': 'GET',
                 'arguments': {'data': '', 'forms': [], 'user_inserts': None},
                 'parsed_request': 'http://www.example.com'
             }))
    ...
    def test_complex_curl_case(self):
        ...
        (self.cleaner._curl_format_case(complex_curl)
             .should.eql({
                'api_url': 'http://www.example.com/api', 
                'method': 'POST',
                'arguments': {'data': "{'auth': {'user': 'abc', 'password': 'def'}}", 'forms': [], 'user_inserts': None},
                'parsed_request': 'http://www.example.com <user_insert>'
             }))
    ...
    def test_mangled_curl_case(self):
        ...
        'original_text': '''
            ...
            \r\napi_key=...
            \r\nkey_secret=...
            \r\nservice_endpoint=\"https://$api_key:$key_secret@text.s4.example.com/v1/news\"
        ...
        (self.cleaner._curl_format_case(mangled_curl)
             .should.eql({
                 'api_url': 'example.com/api', 
                 'method': 'POST',
                 'arguments': {'data': '', 'forms': ['meta="{\'documenttype\' : \'$content_type\'}";type=application/json', 'data=@...'], 'user_inserts': ['$api_key', '$key_secret', '$content_type', '$json_request', '$ms_word_document', '$service_endpoint']},
                 'parsed_request': '"https://$api_key:$key_secret@text.s4.example.com/v1/news" <user_insert>'
             }))
